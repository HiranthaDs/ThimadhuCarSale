// ISO 3166-1 alpha-2 country codes + names. Flag emoji is derived from the code.
const COUNTRY_CODES = {
...
// Emoji flags don't render as flags on every OS (notably Windows), so pages also carry
// a small flag image URL (flagcdn.com, keyed by ISO 3166-1 alpha-2 code) as a fallback.
function flagImageUrl(code) {
  return `https://flagcdn.com/24x18/${code.toLowerCase()}.png`
}

export const COUNTRIES = Object.entries(COUNTRY_CODES)
  .map(([code, name]) => ({ code, name, flag: flagFromCode(code), flagImage: flagImageUrl(code) }))
